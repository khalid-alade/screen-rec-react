...
export default function ScreenRecId() {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        const endpoint = `https://hngtaskfive-production-698b.up.railway.app/api/video/${id}`;
        fetch(endpoint)
            .then((response) => response.json())
            .then((data) => {
                console.log('Server response for video url:', data.data.fileUrl)
